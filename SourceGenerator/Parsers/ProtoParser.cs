...
internal static class ProtoParser
{
    ...
    {
        var rule = tr.ReadNextComment();
        while (ParseComment(rule))
        {
            rule = tr.ReadNextComment();
        }

        var f = new Field(tr, m);
    ...
    static void ParseFieldOption(string key, string val, Field f)
    {
        switch (key)
        {
            case "default":
                ...
                break;
            case "packed":
                f.OptionPacked = bool.Parse(val);
                break;
            case "deprecated":
                f.OptionDeprecated = bool.Parse(val);
                break;
            default:
    ...
    {
        var me = new ProtoEnum(parent, package);
    ...
    {
        var path = tr.ReadNext();
        var publicImport = path == "public";
        if (publicImport)
        {
            path = tr.ReadNext();
        }
